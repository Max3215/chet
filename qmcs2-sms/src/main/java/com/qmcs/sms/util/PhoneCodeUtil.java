...
public class PhoneCodeUtil {
     ...
     * 短信验证码对象失效时间(秒)
     */
    public final static int CODE_OUT_TIME = 300;
     ...
     * @return
     */
    public static String randomSignInCode(){
        StringBuffer code = new StringBuffer();
        //生成短信验证码
        Random random = new Random();
        for (int i = 0; i < 6; i++) {
     ...
     * @return
     */
    public static PhoneCode getInstanceSignInCode(String phone){
        return new PhoneCode(randomSignInCode(),phone, Calendar.getInstance().getTime());
    }
    /**
     * 短信验证码是否有效
     * @return
