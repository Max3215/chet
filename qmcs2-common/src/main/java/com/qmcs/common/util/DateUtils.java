...
public abstract class DateUtils {
     ...
     * @param mill long
     * @return String
     * @Description(功能描述) : 将日期时间戳转换为字符串pattern
     * @author(作者) ： liuchunxu
     * @date (开发日期) : 2015年2月9日 下午3:21:25
     */
    public static String convertString(long mill,String pattern) {
        Date date = new Date(mill);
        String strs = "";
        try {
     ...
     * @param str String
     * @return long
     * @Description(功能描述) : 将字符串pattern转换为时间戳
     * @author(作者) ： liuchunxu
     * @date (开发日期) : 2015年2月9日 下午3:21:25
     */
    public static Long convertLong(String str,String pattern) {
        Calendar c = Calendar.getInstance();
        try {
            c.setTime(new SimpleDateFormat(pattern).parse(str));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return c.getTimeInMillis();
    ...
    /**
     * @return String
     * @Description(功能描述) : 根据年月获取当月最后一天最后一秒
     * @author(作者) ：  suyl
     * @date (开发日期):  2017年6月28日 下午6:11:57
     */
    public static Long lastDayByYearAndMonth(int year,int month) {
        Calendar cale = Calendar.getInstance();
    ...
    /**
     * @return String
     * @Description(功能描述) :  获取当月第一天第一秒
     * @author(作者) ：  suyl
     * @date (开发日期):  2015年5月18日 下午6:12:16
     */
    public static Long firstDayByYearAndMonth(int year,int month) {
        Calendar cal_1 = Calendar.getInstance();//获取当前日期
        cal_1.set(Calendar.YEAR,year);
        cal_1.set(Calendar.MONTH,month-1);
        cal_1.set(Calendar.DAY_OF_MONTH, 1);//设置为1号,当前日期既为本月第一天
        cal_1.set(Calendar.HOUR,00);
